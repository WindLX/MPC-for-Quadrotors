...
class Trajectory:
    ...
    def circle(self, t):
        ...
        if t + dt > T:
            ...
            pos3 = np.array(
                [
                    radius * (np.cos(angle3) - 1),
                    radius * np.sin(angle3),
                    2.5 * angle3 / (2 * np.pi),
                ]
            )
            vel = (pos2 - pos) / dt
    ...
    def diamond(self, t):
        ...
        if (0 < t) and (t <= T1):
            ...
            pos, vel, acc = self.tj_from_line(points[2], points[3], T3, t - (T1 + T2))
    ...
    def get_des_state(self, t):
        ...
        des_state = {"x": pos, "v": vel, "x_ddt": acc, "yaw": 0, "yaw_dot": 0}
        return des_state
